...
namespace Roblox.Common
{
    public static class Extensions
    {
        ...
        {
            var str = stream as MemoryStream;
            if (str != null)
            {
        ...
        {
            Request<CompletionSignal>.HandleResponse(stream.BeginWrite, data, 0, data.Length, (Action<IAsyncResult>)stream.EndWrite, resultHandler);
        }
		...
		public static bool UnorderedEqual<T>(this ICollection<T> list, ICollection<T> listToCompare)
		{
			...
			{
                int num;
                if (d.TryGetValue(key, out num)) d[key] = num + 1;
                else d.Add(key, 1);
            }
			foreach (T key2 in listToCompare)
			{
                int num;
                if (!d.TryGetValue(key2, out num)) return false;
